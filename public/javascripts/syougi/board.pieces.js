...
board.getMovableZone = function (i, j){
  ...
  //香の場合
  if (piece == "香") {
    return getMovableZone2(i,j,piece);

  //飛車・角の場合
  } else if ((piece == "飛") || (piece == "角")) {
    return getMovableZone3(i,j,piece);

  //竜・馬の場合
  } else if ((piece == "竜") || (piece == "馬")) {
    return getMovableZone4(i,j,piece);

  //飛車、角、香以外の駒の場合
  } else {
    return getMovableZone1(i,j,piece);
  }

};

//飛車、角、香以外の駒の場合
var getMovableZone1 = function(i, j, piece){
  //移動可能ゾーンを格納する
  ...
  for (var count = 0; count < board.pieceMovableZone[piece].length; count++ ) {
    ...
    //移動オフセット格納
    if(board.square[i][j].direction == board.playerType.black.direction){
    ...
    if(( xOffset < 0 || 8 < xOffset || yOffset < 0 || 8 < yOffset) == false ) {
      ...
      //移動可能範囲内にあった場合
      if (board.square[yOffset][xOffset].direction != board.player.direction) {
...
//香の場合
var getMovableZone2 = function(i, j, piece){
  //移動可能ゾーンを格納する
  var returnZone = new Array();
  ...
  for (var count = 0; count < board.pieceMovableZone[piece].length; count++ ) {
    ...
    if(( xOffset < 0 || 8 < xOffset || yOffset < 0 || 8 < yOffset) == false ) {
      ...
      if (board.square[yOffset][xOffset].piece == null) {
        ...
        returnZone.push([xOffset,yOffset]);
      //相手の駒の場合
      } else if (board.square[yOffset][xOffset].direction != board.player.direction) {
        //移動可能ゾーンを格納
        returnZone.push([xOffset,yOffset]);
        break;
      //それ以外
...
//飛車・角の場合
var getMovableZone3 = function(i, j, piece){
  //移動可能ゾーンを格納する
  var returnZone = new Array();
...
//竜・馬の場合
var getMovableZone4 = function(i, j, piece){
  //移動可能ゾーンを格納する
  var returnZone = new Array();
